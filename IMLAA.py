...
tf.flags.DEFINE_string('attack_method', 'IMLAA', 'The name of attack method.')
...
tf.flags.DEFINE_string('output_dir', './adv/outputs/', 'Output directory with images.')

tf.flags.DEFINE_float('max_epsilon', 16.0, 'Maximum size of adversarial perturbation.')

tf.flags.DEFINE_integer('num_iter', 15, 'Number of iterations.')

tf.flags.DEFINE_float('alpha', 1.07, 'Step size.')
...
tf.flags.DEFINE_float('scale', 0.25, 'noise deviation')
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        x_grad = tf.gradients(-tf.nn.softmax(logits) * label_ph, adv_input)[0]
        ...
        with tf.Session() as sess:
            ...
            count=0
            for images,names,labels in utils.load_image(FLAGS.input_dir, FLAGS.image_size,FLAGS.batch_size, load_num=1000):
                ...
                for i in tqdm(range(num_iter)):
                    if i==0:
                        ...
                        for l in range(int(FLAGS.ens)):
                            ...
                            c = images_tmp2 + np.random.normal(size = images.shape, loc=0.0, scale=FLAGS.scale)
                            # c = images_tmp2
                            # images_tmp2 = images_tmp2*(1 - l/FLAGS.ens)+ (l/FLAGS.ens)*x_base
                            w, feature, x_grad_real = sess.run([weights_tensor, opt_operations[0], x_grad],feed_dict={ori_input: c, adv_input: c, label_ph: labels})
                            images_tmp2 = images_tmp2 + 0.0025 * np.sign(x_grad_real)
                            weight_np = weight_np + w[:FLAGS.batch_size]

                        # normalize the weights 1. 0.025->0.0025; 2. -normalize -> normalize; 3. del 'weight_np = normalize(weight_np, 2)' line
